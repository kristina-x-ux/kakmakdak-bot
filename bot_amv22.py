...
def notify_admin_group_about_status(chat_id, order_id, status):
    messages = {
        "accepted": f"✅ Заказ {order_id} принят.",
        "on_the_way": f"🚚 Статус заказа «Уже в пути» отправлен клиенту. {order_id}",
        "cancelled": f"❌ Статус заказа «Отменён» отправлен клиенту. {order_id}",
    }
