...
def get_order_meta(order_id):
    if USE_POSTGRES:
        with pg_get_conn() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("""
                    SELECT status, admin_message_chat_id, admin_message_id, accepted_at
                    FROM order_statuses
                    WHERE order_id_text = %s
                """, (order_id,))
                row = cur.fetchone()
...
def mark_order_stale_if_needed(order_id, chat_id=None, message_id=None):
    ...
    ok = safe_edit_message_text(
        ...
        reply_markup=build_admin_status_keyboard(order_id, admin_chat_id, extract_phone_from_text(current_text), "stale"),
        ...
        try:
            if USE_POSTGRES:
                ...
                with pg_get_conn() as conn:
                    with conn.cursor(cursor_factory=RealDictCursor) as cur:
                        cur.execute("""
                            SELECT order_id_text, admin_message_chat_id, admin_message_id
                            FROM order_statuses
...
def set_order_status(order_id, status, admin_message_chat_id=None, admin_message_id=None):
    ...
    if status == "accepted":
        accepted_at_value = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    if USE_POSTGRES:
        with pg_get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO order_statuses (order_id_text, status, admin_message_chat_id, admin_message_id, accepted_at)
                    VALUES (%s, %s, %s, %s, %s)
                    ...
                    DO UPDATE SET
                        ...
                        END,
                        updated_at = NOW()
                """, (order_id, status, admin_message_chat_id, admin_message_id, accepted_at_value))
        return
...
def get_admin_order_keyboard(order_id, client_chat_id, phone, map_link=""):
...
def callbacks(call):
    ...
    if data.startswith("admin_way|"):
        ...
        safe_send_message(
            call.message.chat.id,
            f"🚚 Статус заказа «Уже в пути» отправлен клиенту. {order_id}"
        )
        return
    ...
    if data.startswith("admin_cancel|"):
        ...
        safe_send_message(
            call.message.chat.id,
            f"❌ Статус заказа «Отменён» отправлен клиенту. {order_id}"
        )
        return
...
def finish_order(message, phone, user_id):
    ...
    sent_to_admin = safe_send_message(
        ...
        reply_markup=get_admin_order_keyboard(order_id, message.chat.id, phone, map_link),
        disable_web_page_preview=True
    )
