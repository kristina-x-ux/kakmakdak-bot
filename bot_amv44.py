...
DATA_FILE = "/data/bot_storage.json"
...
RESTAURANT_ROUTE_LAT = 44.4952
RESTAURANT_ROUTE_LON = 34.1663
...
def get_menu_message_id(user_id):
    ...
    return user_data.get("menu_message_id")


def set_menu_message_id(user_id, message_id):
    user_data = get_user_data(user_id)
    user_data["menu_message_id"] = message_id
    save_user_data(user_id, user_data)


def clear_menu_message_id(user_id):
    user_data = get_user_data(user_id)
    user_data.pop("menu_message_id", None)
    save_user_data(user_id, user_data)
                ...
                if meta_row and meta_row.get("meta_value"):
                    try:
                        ...
                        if isinstance(stored_meta, dict):
                            if "admin_base_text" in stored_meta:
                                result["admin_base_text"] = stored_meta["admin_base_text"]
                            if "phone" in stored_meta:
                                result["phone"] = stored_meta["phone"]
                    except Exception:
                        pass
        return result
...
def save_order_meta(order_id, meta):
    allowed_keys = {"admin_base_text", "phone"}

    if USE_POSTGRES:
        filtered = {k: v for k, v in (meta or {}).items() if k in allowed_keys}
        if not filtered:
            return
...
def build_yandex_route_link_to_address(address_text, locality="Не определён", lat=None, lon=None):
    ...
    ВАЖНО: старт маршрута задаём координатами ресторана, а не текстом
    "Ялта, Новый переулок 2". Так Яндекс.Карты открывают именно маршрут
    от ресторана до клиента, а не пустой экран с одним адресом.
    Расстояние и время по дорогам считает само приложение Яндекс.Карт при открытии ссылки.
    """
    origin = f"{RESTAURANT_ROUTE_LAT},{RESTAURANT_ROUTE_LON}"

    if lat is not None and lon is not None:
        # В rtext Яндекс.Карт координаты указываются как широта,долгота.
        destination = f"{float(lat)},{float(lon)}"
        encoded_destination = destination
    else:
        clean_address_text = extract_clean_address_for_maps(address_text)
        explicit_locality = locality if locality in LOCALITIES else detect_locality_from_text(clean_address_text)
        locality_hint = explicit_locality if explicit_locality != "Не определён" else "Ялта"
...
def render_menu_message(chat_id, user_id, text, reply_markup):
    menu_message_id = get_menu_message_id(user_id)
    if menu_message_id:
        ok = safe_edit_message_text(chat_id, menu_message_id, text, reply_markup=reply_markup)
        if ok:
            return menu_message_id
        safe_delete_message(chat_id, menu_message_id)
        clear_menu_message_id(user_id)

    msg = safe_send_message(chat_id, text, reply_markup=reply_markup)
    if msg:
...
def show_categories_message(chat_id, user_id):
    return render_menu_message(chat_id, user_id, "Выбери категорию:", get_categories_keyboard())


def show_cart_message(chat_id, user_id):
    return render_menu_message(chat_id, user_id, format_cart(get_user_cart(user_id)), get_cart_keyboard(user_id))


def show_cart_below_preview(chat_id, user_id):
...
def show_category_message(chat_id, user_id, category_id):
    category_title = MENU[category_id]["title"]
    return render_menu_message(chat_id, user_id, f"Категория: {category_title}", get_items_keyboard(category_id))


def show_sauces_message(chat_id, user_id, category_id, item_name):
    return render_menu_message(chat_id, user_id, f"Для позиции «{item_name}» выбери соус:", get_sauces_keyboard(category_id))


def show_maki_fish_message(chat_id, user_id, category_id):
    return render_menu_message(chat_id, user_id, "Выбери вид маки:", get_maki_fish_keyboard(category_id))


def edit_category_message(call, category_id):
    return show_category_message(call.message.chat.id, call.from_user.id, category_id)


def edit_cart_message(call, user_id):
    return show_cart_message(call.message.chat.id, user_id)
...
def start(message):
    ...
    menu_message_id = get_menu_message_id(user_id)
    if menu_message_id:
        safe_delete_message(message.chat.id, menu_message_id)
...
def show_menu(message):
    show_categories_message(message.chat.id, message.from_user.id)
...
def callbacks(call):
    ...
    if data == "back_categories":
        show_categories_message(call.message.chat.id, user_id)
        preview_message_id = get_preview_message_id(user_id)
        if preview_message_id and call.message.message_id != preview_message_id:
            safe_delete_message(call.message.chat.id, preview_message_id)
            clear_preview_message_id(user_id)
            if get_user_cart(user_id):
                show_or_update_preview(call.message.chat.id, user_id)
        return

    if data == "cart":
        if call.message.message_id == get_preview_message_id(user_id):
            show_cart_below_preview(call.message.chat.id, user_id)
        else:
    ...
    if data.startswith("add|"):
        ...
        if item_name == "Маки с рыбой":
            show_maki_fish_message(call.message.chat.id, user_id, category_id)
            return
        ...
        if item_name in FRYER_ITEMS:
            ...
            show_sauces_message(call.message.chat.id, user_id, category_id, item_name)
            return
        ...
        user_data = get_user_data(user_id)
        if category_id == "7" and user_data.get("awaiting_drink_offer_choice"):
            user_data.pop("awaiting_drink_offer_choice", None)
            save_user_data(user_id, user_data)
    ...
    if data == "drinks_offer_open":
        ...
        menu_message_id = get_menu_message_id(user_id)
        if menu_message_id and menu_message_id != call.message.message_id:
            safe_delete_message(call.message.chat.id, menu_message_id)

        set_menu_message_id(user_id, call.message.message_id)
        safe_edit_message_text(
            call.message.chat.id,
            call.message.message_id,
            f"Категория: {MENU['7']['title']}",
            reply_markup=get_items_keyboard("7")
        )
        return
...
def finish_order(message, phone, user_id):
    ...
    address_full = user_data.get("address_full") or address
    clean_address = user_data.get("clean_address") or user_data.get("address_for_map") or extract_clean_address_for_maps(address_full)
    address_details = user_data.get("address_details", "")
    if not address_details:
        _clean_from_full, _details_from_full = split_address_details(address_full)
        if _details_from_full:
            address_details = _details_from_full

    address_for_map = clean_address or address_full
