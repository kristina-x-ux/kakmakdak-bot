...
def split_address_details(address_text):
    ...
    details = ""
    if clean_address and original.startswith(clean_address):
        details = original[len(clean_address):].strip(" ,.;:-")
    elif clean_address:
        match = re.match(re.escape(clean_address), original, flags=re.IGNORECASE)
        if match:
            details = original[match.end():].strip(" ,.;:-")
