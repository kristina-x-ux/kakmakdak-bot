...
MENU = {
    "1": {
        ...
        "items": {
            ...
            "Шаурма сытная (сыр + картофель по-деревенски)": 500,
            "Хот-дог Датский": 300,
...
def safe_edit_message_text(chat_id, message_id, text, **kwargs):
    try:
        ...
        if "message to edit not found" in error_text:
            logger.warning("edit_message_text skipped: message not found chat_id=%s message_id=%s", chat_id, message_id)
            return False
...
def get_maki_keyboard(category_id):
    kb = types.InlineKeyboardMarkup()
    for maki_name, maki_price in MAKI_VARIANTS.items():
        safe_name = escape_callback_text(maki_name)
        kb.add(types.InlineKeyboardButton(
            f"{maki_name} — {maki_price}₽",
            callback_data=f"maki|{category_id}|{safe_name}"
        ))
    kb.row(
...
def get_chuka_offer_keyboard():
    kb = types.InlineKeyboardMarkup()
    kb.row(
        types.InlineKeyboardButton(f"🥗 Добавить {CHUKA_NAME} — {CHUKA_PRICE}₽", callback_data="offer_chuka_add"),
        types.InlineKeyboardButton("➡️ Продолжить без чуки", callback_data="offer_chuka_skip")
    )
    kb.row(types.InlineKeyboardButton("🛒 Корзина", callback_data="cart"))
    return kb
...
def cart_has_sushi_items(cart):
    sushi_names = set(MENU["4"]["items"].keys()) - {CHUKA_NAME, "Маки с рыбой"}
    sushi_names.update(MAKI_VARIANTS.keys())
    return any(item_name in sushi_names for item_name in cart.keys())


def should_offer_chuka(user_id):
    cart = get_user_cart(user_id)
    if not cart:
        return False
    if CHUKA_NAME in cart:
        return False
    return cart_has_sushi_items(cart)


def begin_checkout(chat_id, user_id):
    cart = get_user_cart(user_id)
    if not cart:
    ...
    if should_offer_chuka(user_id):
        safe_send_message(
            chat_id,
            f"К заказу можно добавить {CHUKA_NAME} — {CHUKA_PRICE}₽. Добавить?",
...
def cancel_current_flow(message):
    ...
    reset_user_flow(user_id)
    user_data = get_user_data(user_id)
    if user_data.get("pending_fryer_item"):
        user_data.pop("pending_fryer_item", None)
        save_user_data(user_id, user_data)
    safe_send_message(message.chat.id, "❌ Действие отменено.", reply_markup=get_main_keyboard())
...
def checkout_from_main(message):
    begin_checkout(message.chat.id, message.from_user.id)
...
def callbacks(call):
    ...
    if data == "checkout":
        begin_checkout(call.message.chat.id, user_id)
        return
    ...
    if data.startswith("maki|"):
        _, category_id, safe_maki_name = data.split("|", 2)
        maki_name = unescape_callback_text(safe_maki_name)
        maki_price = MAKI_VARIANTS.get(maki_name)
        if maki_price is None:
            safe_send_message(call.message.chat.id, "Ошибка: вариант маки не найден.")
            return
        add_to_cart(user_id, maki_name, maki_price)
        show_or_update_preview(call.message.chat.id, user_id, maki_name)
        edit_category_message(call, category_id)
        return

    if data == "offer_chuka_add":
        add_to_cart(user_id, CHUKA_NAME, CHUKA_PRICE)
        show_or_update_preview(call.message.chat.id, user_id, CHUKA_NAME)
        safe_send_message(call.message.chat.id, f"{CHUKA_NAME} добавлен в корзину.")
        start_address_step(call.message.chat.id, user_id)
        return

    if data == "offer_chuka_skip":
        start_address_step(call.message.chat.id, user_id)
        return
    ...
    if data.startswith("add|"):
        ...
        if item_name == "Маки с рыбой":
            ok = safe_edit_message_text(
                ...
                call.message.message_id,
                "Выбери вид маки:",
                reply_markup=get_maki_keyboard(category_id)
            )
            if not ok:
                safe_send_message(
                    call.message.chat.id,
...
def public_post_worker():
    while True:
        try:
            post_public_info_if_needed()
        except Exception:
            logger.exception("public_post_worker failed")
        time.sleep(30)
...
def run_bot():
    ...
    threading.Thread(target=public_post_worker, daemon=True).start()
